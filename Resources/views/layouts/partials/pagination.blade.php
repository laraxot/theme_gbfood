<ul class="pagination" itemscope itemtype="http://schema.org/SiteNavigationElement/Pagination">
    ...
    @else
        <li class="page-item">
            <a class="page-link" tabindex="-1" aria-label="Previous" href="{{ $paginator->previousPageUrl() }}"
                rel="prev" itemprop="relatedLink/pagination">
                <span aria-hidden="true">&laquo;</span> <span class="sr-only">Previous</span>
            </a>
        </li>
    @endif 
        ...
        @if (is_array($element))
            @foreach ($element as $page => $url)
                @if ($page == $paginator->currentPage())
                    <li class="page-item active"> <a class="page-link" href="{{ $url }}">{{ $page }} <span
